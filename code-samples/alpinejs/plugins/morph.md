...
**Source URL:** https://alpinejs.dev/plugins/morph
...
Here's an example of using `Alpine.morph()` to update an Alpine component with new HTML:
...
| Parameter | Description |
|-----------|-------------|
| `el` | The actual, current, DOM element on the page that will be "patched" (changed by Morph). |
| `toEl` | A "template element". It's a temporary element representing what the live el will be patched to. It will never actually live on the page and should only be used for reference purposes. |
| `childrenOnly()` | A function that can be called inside the hook to tell Morph to skip the current element and only "patch" its children. |
...
The most common use-case for them is a list of siblings within a loop. By adding keys to each node, Morph will match them in both trees and move them accordingly.
