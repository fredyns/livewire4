...
Below is a simple `CountDown` component that displays a count-down from `3` to `0` when a button is pressed:



```php
use Livewire\Component;class CountDown extends Component{    public $start = 3;    public function begin()    {        while ($this->start >= 0) {            // Stream the current count to the browser...            $this->stream(                to: 'count',                content: $this->start,                replace: true,            );            // Pause for 1 second between numbers...            sleep(1);            // Decrement the counter...            $this->start = $this->start
...
A common use-case for `wire:stream` is streaming chat-bot responses as they are received from an API that supports streamed responses.Below is an example of using `wire:stream` to accomplish a ChatGPT-like interface:
...
```blade
<!-- Append contents... --><div wire:stream="target"></div><!-- Replace contents... --><div wire:stream.replace="target"></div>
