...
```php
<?php// resources/views/components/Ã¢Å¡Â¡revenue.blade.phpuse Livewire\Attributes\Lazy;use Livewire\Component;use App\Models\Transaction;new #[Lazy] class extends Component {    public $amount;    public function mount()    {        // Slow database query...        $this->amount = Transaction::monthToDate()->sum('amount');    }};?><div>    Revenue this month: {{ $amount }}</div>

```



With `#[Lazy]`, the component initially renders as an empty `<div></div>`, then loads when it enters the viewport Ã¢â‚¬â€ typically when a user scrolls to it.
...
```php
<?php// resources/views/components/Ã¢Å¡Â¡revenue.blade.phpuse Livewire\Attributes\Lazy;use Livewire\Component;new #[Lazy(bundle: true)] class extends Component {    // ...};
...
Use `#[Lazy]` when:- Components contain slow operations (database queries, API calls) that would delay page load- The component is below the fold and users might not scroll to it- You want to improve perceived performance by showing the page faster- You have multiple expensive components on a single page
