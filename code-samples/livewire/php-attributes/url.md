...
The `#[Url]` attribute does two things:1. Writes to URL
...
Both `#[Url]` and `#[Session]` persist property values, but with different trade-offs.Use `#[Url]` when you want users to be able to share or bookmark the current state.Use `#[Session]` when state should be private.
...
```php
<?php// resources/views/components/user/Ã¢Å¡Â¡index.blade.phpuse Livewire\Attributes\Url;use Livewire\Component;new class extends Component {    #[Url(as: 'q')]    public $search = '';};
...
```php
<?php// resources/views/components/user/Ã¢Å¡Â¡index.blade.phpuse Livewire\Attributes\Url;use Livewire\Component;new class extends Component {    #[Url(except: '')]    public $search = '';    public function mount()    {        $this->search = auth()->user()->username;    }};
...
```php
<?php// resources/views/components/user/Ã¢Å¡Â¡index.blade.phpuse Livewire\Attributes\Url;use Livewire\Component;new class extends Component {    #[Url]    public ?string $search;};
...
Use `#[Url]` when:- Building search or filter interfaces- Implementing pagination- Creating shareable views (map positions, selected filters, etc.)- Allowing users to bookmark specific states- Supporting browser back/forward navigation through states
...
- Good for SEO- Analytics tracking- Shareable on social media
...
```text
#[Url(    ?string $as = null,    bool $history = false,    bool $keep = false,    mixed $except = null,    mixed $nullable = null,)]
