...
**Source URL:** https://livewire.laravel.com/docs/4.x/attribute-session
...
**Don't store large amounts of data.** Laravel sessions are loaded into memory during every request. Storing too much in a user's session can slow down your entire application for that user. Avoid storing large collections or objects.
...
**Alternative: URL persistence.** If you want state to be shareable via URL or bookmarkable, consider using the `#[Url]` attribute instead of `#[Session]`. URL parameters persist state in the address bar while session properties keep the URL clean.
...
- [Url](./url.md) — Persist state in URL query parameters
