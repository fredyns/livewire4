...
```php
<?php// resources/views/components/post/Ã¢Å¡Â¡show.blade.phpuse Livewire\Attributes\Async;use Livewire\Component;use App\Models\Post;new class extends Component {    public Post $post;    #[Async]    public function logActivity()    {        Activity::log('post-viewed', $this->post);    }};
...
```blade
<div wire:intersect="logActivity">    <!-- Logs activity asynchronously when element enters viewport --></div>
...
Use `#[Async]` for fire-and-forget operations where the result doesn't affect what's displayed on the page:- Analytics and logging
...
```php
<?php// resources/views/components/Ã¢Å¡Â¡external-link.blade.phpuse Livewire\Attributes\Async;use Livewire\Component;new class extends Component {    public $url;    #[Async]    public function trackClick()    {        Analytics::track('external-link-clicked', [            'url' => $this->url,            'user_id' => auth()->id(),        ]);    }};
...
```blade
<a href="{{ $url }}" target="_blank" wire:click="trackClick">    Visit External Site</a>
...
Never use async actions if they modify component state that's reflected in your UI.Because async actions run in parallel, you can end up with unpredictable race conditions where your component's state diverges across multiple simultaneous requests.Dangerous example:



```php
// Warning: This snippet demonstrates what NOT to do...<?php// resources/views/components/Ã¢Å¡Â¡counter.blade.phpuse Livewire\Attributes\Async;use Livewire\Component;new class extends Component {    public $count = 0;    #[Async] // Don't do this!    public function increment()    {        $this->count++; // State mutation in an async action    }};

```



If a user rapidly clicks the increment button, multiple async requests fire simultaneously. Each request starts with the same initial `$count` value, leading to lost updates.The rule of thumb: only use async for actions that perform pure side effects Ã¢â‚¬â€ operations that don't change any properties that affect your component's view.
