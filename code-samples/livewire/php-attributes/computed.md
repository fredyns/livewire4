...
**Source URL:** https://livewire.laravel.com/docs/4.x/attribute-computed
...
**Note:** Unlike normal properties, computed properties must be accessed via `$this` in your template. For example, `$this->posts` instead of `$posts`.
...
**Not supported on Form objects:** Computed properties cannot be used on `Livewire\Form` objects. Attempting to access them via `$form->property` will result in an error.
...
- [Computed Properties](../features/computed-properties.md) — Detailed guide to computed properties
