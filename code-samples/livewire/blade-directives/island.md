...
```blade
@island(name: 'revenue')    <div>Revenue: {{ $this->revenue }}</div>@endisland<button type="button" wire:click="$refresh" wire:island="revenue">    Refresh revenue</button>
...
```text
@island(    ?string $name = null,    bool $lazy = false,    bool $defer = false,)    <!-- Content -->@endisland
