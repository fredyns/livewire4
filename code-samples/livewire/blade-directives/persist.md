...
When navigating with `wire:navigate`:1. Livewire looks for elements with matching `@persist` names on both pages2. If found, the existing element is moved to the new page's DOM3. The element's state, event listeners, and Alpine data are preservedLearn more: 
