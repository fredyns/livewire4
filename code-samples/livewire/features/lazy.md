...
**Source URL:** https://livewire.laravel.com/docs/4.x/lazy
...
**Note:** Lazy and deferred requests are isolated by default. Unlike other network requests in Livewire, lazy and deferred component updates are isolated from each other when sent to the server. This keeps loading fast by loading each component in parallel.
...
**Important:** The `@placeholder` directive is only available for view-based components (single-file and multi-file components). For class-based components, use the `placeholder()` method instead.

**Important:** The placeholder and the component must share the same element type. For instance, if your placeholder's root element type is a 'div,' your component must also use a 'div' element.
...
**Legacy syntax:** You can also use `lazy="on-load"` which behaves the same as `defer`. The `defer` parameter is recommended for new code.
...
**Legacy syntax:** You can also use `isolate: false` which behaves the same as `bundle: true`. The `bundle` parameter is recommended for new code as it's more explicit about the intent.
...
- [@placeholder](../blade-directives/placeholder.md) — Define placeholder content
- [Lazy Attribute](../php-attributes/lazy.md) — Mark components for lazy loading
