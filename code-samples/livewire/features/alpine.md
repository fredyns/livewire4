...
You can also pass parameters to Livewire methods by simply passing them to the `$wire` method call.Consider a component with a `deletePost()` method like so:
...
```blade
@foreach ($posts as $post)    <button        type="button"        wire:key="{{ $post->id }}"        x-on:click="$wire.deletePost({{ $post->id }})"    >        Delete "{{ $post->title }}"    </button>@endforeach
...
```html
<div x-data="{ open: $wire.entangle('showDropdown') }">    <button x-on:click="open = true">Show More...</button>    <ul x-show="open">        <li><button wire:click="archive">Archive</button></li>    </ul></div>
...
# Manually bundling Alpine in your Java

Script buildBy default, Livewire and Alpine's JavaScript is injected onto each Livewire page automatically.This is ideal for simpler setups, however, you may want to include your own Alpine components, stores, and plugins into your project.To include Livewire and Alpine via your own JavaScript bundle on a page is straightforward.First, you must include the `@livewireScriptConfig` directive in your layout file like so:
...
# See also
- [Properties](../essentials/properties.md) Ã¢â‚¬â€ Access Livewire properties from Alpine using $wire- [Actions](../essentials/actions.md) Ã¢â‚¬â€ Call Livewire actions from Alpine- [JavaScript](../advanced/javascript.md) Ã¢â‚¬â€ Execute custom JavaScript in components- [Events](../essentials/events.md) Ã¢â‚¬â€ Dispatch and listen for events with Alpine
